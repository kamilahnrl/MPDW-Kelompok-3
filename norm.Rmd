...
# Menggunakan fungsi Min-Max scaling pada dataset
data <- min_max(data)
print(data)
```
...
```{r}
sisaan3<-garchFit12@fit[["residuals"]]
ks.test(sisaan3,"pnorm")
Box.test(sisaan3, type = "Ljung")
Box.test((sisaan3)^2, type = "Ljung") 
```
...
garchSpec22 <- ugarchspec(
  variance.model=list(model="sGARCH",
                      garchOrder=c(2,2)),
mean.model=list(armaOrder=c(1,0)))
garchFit22 <- ugarchfit(spec=garchSpec22, data=train.ts)
garchFit22
```
...
garchSpec13 <- ugarchspec(
  variance.model=list(model="sGARCH",
                      garchOrder=c(1,3)),
mean.model=list(armaOrder=c(1,0)))
garchFit13 <- ugarchfit(spec=garchSpec13, data=train.ts)
garchFit13
```
...
```{r}
forecastValues2 <- as.numeric(forc@forecast$seriesFor)
```
...
```{r}
perbandingan<-matrix(data=c(head(test, n=45), forecastValues2),
                     nrow = 45, ncol = 2)
colnames(perbandingan)<-c("Aktual","Hasil Forecast")
perbandingan
accuracy(ts(forecastValues2), head(test, n=45))
```
normalisasai ARIMA GARCH
MAPE = 13.5529


```{r}
ramalan<- forecast::forecast(model1, h = 45) 
data.ramalan <- ramalan$mean
perbandingan<-matrix(data=c(head(test, n=45), data.ramalan),
                     nrow = 45, ncol = 2)
colnames(perbandingan)<-c("Aktual","Hasil Forecast")
perbandingan
accuracy(ts(data.ramalan), head(test, n=45))
```
normalisasi ARIMA
MAPE=16.86416
...
library(ggplot2)
waktu<-c(1:45)
data5<-data.frame(test,forecastValues2,waktu)

plot2 <- ggplot() +
  geom_line(data5,mapping=aes(x=waktu,y=test), color="red")+
  geom_line(data5,mapping=aes(x=waktu,y=forecastValues2), color="blue")+
  theme_classic()
plot2
```
...
plot3 <- ggplot() +
  geom_line(data5,mapping=aes(x=waktu,y=test), color="red")+
  geom_line(data5,mapping=aes(x=waktu,y=data.ramalan), color="green")+
  theme_classic()
plot3
